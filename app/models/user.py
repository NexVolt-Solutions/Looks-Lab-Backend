...
class User(Base):
    ...
    last_google_id_token: Mapped[str | None] = mapped_column(String(2048), nullable=True)
    last_apple_id_token: Mapped[str | None] = mapped_column(String(2048), nullable=True)
    ...
    selected_domain: Mapped[str | None] = mapped_column(String(50), nullable=True)
    ...
    domain_answers: Mapped[list["DomainAnswer"]] = relationship(
        "DomainAnswer", back_populates="user", cascade="all, delete-orphan"
    )
